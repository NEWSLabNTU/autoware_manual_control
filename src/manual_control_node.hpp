...
class ManualControlNode : public rclcpp::Node
{
  ...
  private:
    ...
    void publish_cmd()
    {
      Control control;
      {
        control.stamp = this->get_clock()->now();
        control.lateral.steering_tire_angle = steering_tire_angle_;
        ...
        control.longitudinal.velocity = real_target_velocity_;
      ...
      pub_control_command_->publish(control);
